...
class RuleBasedRoadmapGenerator {
  constructor() {
    ...
    // Week-to-week progression inside a phase (helps avoid duplicate content)
    this.weekProgressionStages = [
      {
        key: 'intro',
        label: 'Foundation Intro',
        objectiveHints: [
          'Understand fundamentals of',
          'Identify core building blocks of',
        ],
        projectLabel: 'Starter Lab',
        projectDescription: 'Create a simple {skill} lab to reinforce the basics you just covered.',
      ...
      },
      {
        ...
        label: 'Hands-on Lab',
        objectiveHints: [
          'Apply essential commands of',
          'Practice core workflows with',
        ],
        projectLabel: 'Guided Lab',
        projectDescription: 'Follow a guided lab to implement {skill} features end‑to‑end.',
        projectDifficulty: 'intermediate',
        projectTime: '6-10 hours',
      ...
      },
      {
        ...
        objectiveHints: [
          'Build real-world features using',
          'Integrate {skill} with complementary technologies for a project',
        ],
        projectLabel: 'Project Sprint',
        projectDescription: 'Ship a functional {skill} project that mirrors a production scenario.',
        projectDifficulty: 'intermediate',
        projectTime: '10-15 hours',
        assessmentType: 'project',
      },
      {
        key: 'optimization',
        label: 'Optimization & Review',
        objectiveHints: [
          'Optimize performance and scalability for',
          'Review advanced patterns in',
        ],
        projectLabel: 'Optimization Challenge',
        projectDescription: 'Refine an existing {skill} solution focusing on performance and resilience.',
        projectDifficulty: 'advanced',
        projectTime: '12-18 hours',
  ...
  /**
   * Determine intra-phase progression stage (intro → lab → project → optimization)
   */
  _getProgressionStage(weekIndex, totalWeeksInPhase) {
    if (totalWeeksInPhase <= this.weekProgressionStages.length) {
      return this.weekProgressionStages[Math.min(weekIndex - 1, this.weekProgressionStages.length - 1)];
    }

    const ratio = (weekIndex - 1) / Math.max(totalWeeksInPhase - 1, 1);
    if (ratio < 0.25) return this.weekProgressionStages[0];
    if (ratio < 0.5) return this.weekProgressionStages[1];
    if (ratio < 0.75) return this.weekProgressionStages[2];
    return this.weekProgressionStages[3];
  }

  /**
   * Enhance week content so repeated skills feel progressive instead of duplicated
   */
  _applyProgressionEnhancements(week, weekSkills, stage, phase, currentLevel = 'beginner') {
    ...
    week.progressionStage = stage.key;
    ...
    week.focus = `${skillName} - ${stage.label}`;

    const stageObjectives = (stage.objectiveHints || []).map((hint) => {
      const template = hint.includes('{skill}')
        ? hint.replace('{skill}', skillName)
        : `${hint} ${skillName}`;
      return template.replace(/\s+/g, ' ').trim();
    });

    week.learningObjectives = this._mergeUniqueObjectives(
      [...(week.learningObjectives || []), ...stageObjectives],
      6
    );

    week.projects = this._adjustProjectsForStage(
      week.projects,
      skillName,
      stage,
      phase,
    ...
    week.assessments = this._adjustAssessmentsForStage(
      week.assessments,
      skillName,
      stage,
      currentLevel
  ...
  _mergeUniqueObjectives(objectives, limit = 5) {
    const result = [];
    const seen = new Set();
  ...
  _adjustProjectsForStage(projects = [], skillName, stage, phase, currentLevel) {
    const cloned = projects.length > 0 ? [...projects] : [];
    const baseProject = cloned[0] ? { ...cloned[0] } : null;
    const title = stage.projectLabel
      ? `${skillName} ${stage.projectLabel}`
      : baseProject?.title || `${skillName} Project`;
    const descriptionTemplate =
      stage.projectDescription ||
      baseProject?.description ||
      `Apply ${skillName} concepts in a practical scenario.`;
    const description = descriptionTemplate.replace('{skill}', skillName);
    const difficulty = stage.projectDifficulty || baseProject?.difficulty || (phase?.phaseNumber >= 3 ? 'advanced' : 'intermediate');
    const estimatedTime = stage.projectTime || baseProject?.estimatedTime || '10-15 hours';
    ...
    const updatedProject = {
      ...
      difficulty,
      estimatedTime,
      skills: baseProject?.skills || [skillName],
    };

    if (cloned.length === 0) {
      cloned.push(updatedProject);
    } else {
      cloned[0] = updatedProject;
    }

    return cloned;
  }

  _adjustAssessmentsForStage(assessments = [], skillName, stage, currentLevel) {
    const cloned = assessments.length > 0 ? [...assessments] : [];
    const stagedAssessment = this._createStageAssessment(skillName, stage.assessmentType, currentLevel);

    if (stagedAssessment) {
      if (cloned.length === 0) {
        cloned.push(stagedAssessment);
      } else {
        cloned[0] = stagedAssessment;
      }
    }
  ...
  _createStageAssessment(skillName, assessmentType = 'project', currentLevel = 'beginner') {
    if (assessmentType === 'quiz') {
      return {
        type: 'quiz',
        description: `Complete a ${skillName} fundamentals quiz to verify understanding of key concepts.`,
        passingCriteria: 'Score at least 70%',
      };
    }
    ...
    if (assessmentType === 'coding-challenge') {
      return {
        type: 'coding-challenge',
        description: `Solve an advanced ${skillName} optimization challenge focusing on performance and scalability.`,
        passingCriteria: currentLevel === 'advanced'
          ? 'Deliver an optimal, production-grade solution'
          : 'Deliver a working, efficient solution',
      };
    }

    return {
      type: 'project',
      description: `Complete the ${skillName} lab or project and submit a working implementation.`,
      passingCriteria: 'Working project with all required features implemented',
    };
  }
}
