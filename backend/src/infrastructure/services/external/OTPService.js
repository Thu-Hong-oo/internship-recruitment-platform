const {
  logger,
} = require('d:/KhoaLuan_Internship/internship-recruitment-platform/backend/src/shared/utils/logger');
...
class OTPService {
  ...
  checkInitialized() {
    if (!this.initialized || !this.redisClient) {
      throw new Error(
        'OTP Service not initialized or Redis client not available'
      );
    }
  }
