...
const { User, Candidate, Employer } = require('../../domain/identity');
const { logger } = require('../../shared/utils/logger');
const { sanitizeInput } = require('../../shared/utils/verificationValidation');
...
class UnifiedProfileService {
  ...
  static async updateCandidateProfile(userId, updates) {
    ...
    if (!profile) {
      profile = await Candidate.create({ userId });
    }
  ...
  static async updateEmployerProfile(userId, updates) {
    let profile = await Employer.findOne({ owner: userId });
    if (!profile) {
