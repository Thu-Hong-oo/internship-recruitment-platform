...
const setupSocket = server => {
  ...
  const io = new Server(server, {
    cors: {
      //bật cor để client có thể kết nối từ domain khác
  ...
  io.on('connection', socket => {
    ...
    socket.on('application-status-update', data => {
      ...
      socket
        ...
        .emit('application-status-changed', {
          applicationId,
          status,
          message,
          timestamp: new Date(),
    ...
    socket.on('interview-scheduled', data => {
      ...
      if (applicantId) {
        io.to(`user:${applicantId}`).emit('interview-notification', {
          ...
          applicationId,
          interviewData,
          message: 'Bạn có một lịch phỏng vấn mới',
        });
      }
      ...
      if (employerId) {
        io.to(`user:${employerId}`).emit('interview-notification', {
          ...
          applicationId,
          interviewData,
          message: 'Lịch phỏng vấn đã được lên kế hoạch',
        });
      }
      ...
      try {
        ...
        setTimeout(() => {
          socket.emit('recommendation-progress', {
            status: 'processing',
            progress: 50,
            message: 'Đang so khớp với các công việc có sẵn...',
          });
        }, 1000);
        ...
        setTimeout(() => {
          socket.emit('recommendations-ready', {
            status: 'completed',
            message: 'Đề xuất công việc đã sẵn sàng!',
